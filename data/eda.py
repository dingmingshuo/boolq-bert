...
def get_synonyms(word):
    ...
    for syn in wordnet.synsets(word): 
        for l in syn.lemmas(): 
            ...
            if '_' in synonym: continue
            if '-' in synonym: continue
...
def eda(length,words, mask,sr_rate,rd_rate,rs_rate,ri_rate):
    words_da = [words]
    mask_da = [mask]
    l = list(mask).index(0) if 0 in mask else length

    if(sr_rate > 0):
        words_da.append(synonym_replacement(words,int(sr_rate*l)))
        mask_da.append(mask)
    ...
    if(rd_rate > 0):
        ...
        new_mask = mask[delta:]
        new_mask.extend([0 for _ in range(delta)])
        words_da.append(new_words)
        mask_da.append(new_mask)

    if(rs_rate > 0):
        words_da.append(random_swap(words,int(rs_rate*l)))
        mask_da.append(mask)

    if(ri_rate > 0):
        new_words = random_insertion(words,int(ri_rate*l),l)[:length]
        if not new_words[-1] == '[PAD]':
            new_words[-1] = '[SEP]'
        nl = int(ri_rate*l)+l
        if nl < length:
            new_mask = [1 for _ in range(nl)]
            new_mask.extend([0 for _ in range(length-nl)])
        else:
            new_mask =  [1 for _ in range(length)]
        words_da.append(new_words)
        mask_da.append(new_mask)
